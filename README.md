...
- 导入自 Socratopia：自动连接本地数据并列出当前课程与每堂课堂记录；通过 Socratopia 本地只读接口取得 qbook 原始教材，按所选课堂的实际进度、教学效果和既有卡片综合制卡，并排除预加载的未来章节
- OpenAI SDK 封装：支持自定义 `base_url`、模型和 API 密钥，以及 Ollama 本地模式
- `mss` 全局截图热键（默认 `Alt+S`，可在设置中修改）与多显示器选区，默认选区完成后立即生成卡片，支持多模态直传或可选的本地 RapidOCR 文字降级
- 将模型 JSON 严格解析为 QA、Cloze、判断、单选、多选五类卡片；预览中支持编辑、重新生成、删除单张或确认后删除全部卡片
- AnkiConnect：可刷新、选择或手动输入目标牌组；自动创建牌组、注册独立命名的 FlashForge 笔记类型，提供五种题型化卡面、暗色与移动端适配，重试时跳过已存在卡片
- 单张编辑、删除和带反馈的重新生成
...
3. 在“制卡”页粘贴材料、导入本地 Markdown、点击“导入自 Socratopia”选择课程和课堂，或按已配置的快捷键截取材料区域；点击“生成卡片”。课堂记录会启用专用提炼规则。
4. 在预览区选中卡片，可以编辑、删除或要求模型重新生成一张。
5. 选择目标牌组，点击“导入 Anki”。状态栏会显示逐张进度；重复导入会跳过已有卡片。

点击“导入自 Socratopia”后，FlashForge 会自动定位 `%APPDATA%\socratopia\project-data\accounts`，无需手动选择账户目录。选择窗口将当前课程排在前面，并显示每堂课的时间、标题、学习时长、消息数、课堂总结和课后教学效果。选择一堂课后，FlashForge 不直接读取或保存 qbook 密钥，而是通过 Socratopia 的本地只读接口取得该课堂对应的已解密章节：先根据所选课堂记录确定实际授课终点，再回溯到本章开头，完整纳入这段 qbook 原文。这样课堂一笔带过但属于已讲范围的核心知识仍会参与制卡；课堂错误与纠正负责调整优先级，已有卡片负责去重。使用 qbook 原文时请保持 Socratopia 正在运行；若本地接口不可用，程序会明确提示并回退到课堂教材缓存。
